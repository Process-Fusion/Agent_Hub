...
class ClassificationAgentState(State):
  ...
  keyword_ids: List[int] = Field(default = [], description="The IDs of the keywords extracted from the document")
  document_name: str = Field(description="The name of the document")
  next_step: Optional[str] = Field(default=None, description="The next step of the workflow")
  trust_routing: str = Field(default="", description="The trust routing of the workflow")
  human_correction: Optional[str] = Field(default=None, description="The human correction classification type")
